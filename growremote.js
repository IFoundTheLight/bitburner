...
export async function main(ns) {
  ...
  while(true)
  {
    ...
    let totalMultipler = multiplierNeeded / growthMultipler;
    ...
    // Growing message
    await ns.tprint(`Growing money on ${remoteHost}, grow will take ${growTimeinTime}, threads needed ${threadCountForGrowth} [${maxMoney}, ${currentMoney}, ${multiplierNeeded}, ${growthMultipler}, ${totalMultipler}]`);

    // grow
    let grownAmount = await ns.grow(remoteHost, {threads: Number(threadCountForGrowth)});

    // grown
    await ns.tprint(`${remoteHost} grew by ${ns.formatNumber(grownAmount, 2, 1000, false)}`);
  }
}
